...
that require access to a database, etc... .
Many of the practices of the advice given in this vignette do not
...
Unit tests (the lowest level of tests) are relatively easy to classify. For the
other levels of tests there is more dissent in the literature.
...
gived inputs the expected outputs are produced, without knowledge of what 
happens in between.
...
would be *boss-level* (i.e. test the goals set by your boss). 
...
Your boss tells you to reimplement an analysis workflow from SAS code to R. 
...
programmatically. They do not fit in the hirachy above but a rather parallel to
...
```
# A Proposed Folder Structure for Tests
