...
Test a small unit of code from the programer's perspective.
...
as *programer-level*, and functional tests as *user-level*, acceptance tests 
...
programatically. They do not fit in the hierarchy above but a rather parallel to
...
# Organizing the tests/testthat directory

When your tests start becoming longer than you want them to be, there are
two basic approaches to organize them

  * Dividide them into short and long-running tests
    * Advantage: You can still run all tests that perform well enough, 
      regardless of whether they are acceptance, functional, unit, etc..
    * Disadvantage: The structure of your tests suite is a bit more arbitrary
      and it might be harder to stay organized
  * Divide them by class of test (see next chapter)
    * Neat subdivision of testing principle, forces you think about functional
      tests vs. unit tests.
    * Functional and acceptance tests that would run quick enough to be 
      executed regularily will only be executed with your long-runing tests
...
* **/test/testthat/testdata**: Data necessary for tests goes here. Binary data
...
* **/test/testthat/testdata-raw**: Contains scripts that create the files in 
  testdata; similar to the `data-raw` directory proposed in @wickham2015
...
If you use them, *testdata* and *test_out* need to be added to your 
